class WalletGreyCard
  ...
  belongs_to :grey_card, foreign_key: :grey_card_id, primary_key: "uid"

  index({wallet_addr: 1, grey_card_id: 1}, {name: "wallet_addr_grey_card_id_index", unique: true, background: true})
  index({wallet_addr: 1}, {name: "wallet_addr_index", background: true})
  index({grey_card_id: 1}, {name: "grey_card_id_index", background: true})
  ...
  validates :grey_card_id, uniqueness: {scope: :wallet_addr}
  validates :wallet_addr, :grey_card_id, presence: true

  validate :validate_grey_card_id

  def validate_grey_card_id
    permitted_grey_card_ids = GreyCard.distinct(:uid)
    errors.add(:grey_card_id, "Unpermitted uid") unless permitted_grey_card_ids.include?(grey_card_id)
  end
end
