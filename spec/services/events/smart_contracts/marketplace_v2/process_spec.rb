...
RSpec.describe Events::SmartContracts::MarketplaceV2::Process do
  it "calls the correct service based on event name (sale created)" do
    allow(Events::SmartContracts::ValidateTransaction).to receive(:call).and_return(true)
    allow(Events::SmartContracts::MarketplaceV2::SaleCreated).to receive(:call)
    described_class.call({event_name: "SaleCreated"}.stringify_keys)
    expect(Events::SmartContracts::MarketplaceV2::SaleCreated).to have_received(:call)
  end
  ...
  it "does not call the event handler if the transaction is invalid" do
    ...
    expect(Events::SmartContracts::MarketplaceV2::SaleCreated).not_to receive(:call)
