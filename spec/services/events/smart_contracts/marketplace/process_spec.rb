...
RSpec.describe Events::SmartContracts::Marketplace::Process do
  it "calls the correct service based on event name (sale created)" do
    ...
    allow(Events::SmartContracts::Marketplace::SaleCreated).to receive(:call)
    described_class.call({event_name: "SaleCreated"}.stringify_keys)
    expect(Events::SmartContracts::Marketplace::SaleCreated).to have_received(:call)
  end
  ...
  it "does not call the event handler if the transaction is invalid" do
    ...
    expect(Events::SmartContracts::Marketplace::SaleCreated).not_to receive(:call)
