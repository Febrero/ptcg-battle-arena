...
RSpec.describe SendGameEventToKafka do
  let(:game_event) do
  ...
  it "should remove players with a nil wallet_addr from the event posted to kafka" do
    ...
    expect(KafkaIntegration::Producer).to have_received(:deliver).with(kafka_event.to_json, topic: "events.games.battle_arena").once
  end
end
